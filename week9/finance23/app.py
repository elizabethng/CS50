...
def buy():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("shares"):
            return apology("must provide shares > 0", 403)

        shares = float(request.form.get("shares")) # not sure if int or float is correct for API
        if shares <= 0:
            return apology("must provide shares > 0", 403)
...
def register():
    ...
    # Forget any user_id - not sure if needed
    # Probably want it to say already registered if they try from logged in session?
    # session.clear()
